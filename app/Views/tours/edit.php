...
?>
    ...
    <style>
        ...
        #drop-zone.drag-over { border-color: var(--media-accent); background: rgba(108,99,255,.06); transform: scale(1.01); }
        #drop-zone.drag-over::before { opacity: 1; }
        #drop-zone .drop-icon { font-size: 2.4rem; margin-bottom: .6rem; display: block; animation: float 3s ease-in-out infinite; }
        @keyframes float { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-6px); } }
        #drop-zone .drop-title { font-size: 1rem; font-weight: 600; color: var(--media-text); margin-bottom: .25rem; }
        #drop-zone .drop-sub { font-size: .8rem; color: var(--media-muted); margin: 0; }
        ...
        .media-thumb { width: 100%; aspect-ratio: 16/9; object-fit: cover; display: block; background: #0a0c12; }
        .video-thumb { width: 100%; aspect-ratio: 16/9; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg,#0a0c12,#1a1d27); font-size: 2.5rem; color: var(--media-accent); }

        .media-type-badge { position: absolute; top: .5rem; left: .5rem; background: rgba(0,0,0,.65); backdrop-filter: blur(6px); color: #fff; font-size: .65rem; font-weight: 700; letter-spacing: .06em; text-transform: uppercase; padding: .25rem .6rem; border-radius: 20px; border: 1px solid rgba(255,255,255,.15); }
        .media-type-badge.video { background: rgba(108,99,255,.75); }
        .media-type-badge.image { background: rgba(0,212,170,.55); }

        .btn-delete-media { position: absolute; top: .5rem; right: .5rem; width: 28px; height: 28px; border-radius: 50%; background: rgba(255,77,109,.85); border: none; color: #fff; font-size: .9rem; display: flex; align-items: center; justify-content: center; cursor: pointer; opacity: 0; transform: scale(.8); transition: opacity .2s, transform .2s; }
        .media-card:hover .btn-delete-media { opacity: 1; transform: scale(1); }

        .drag-handle { position: absolute; bottom: .5rem; right: .5rem; color: var(--media-muted); font-size: .85rem; cursor: grab; opacity: 0; transition: opacity .2s; }
        .media-card:hover .drag-handle { opacity: 1; }
        .media-card.sortable-ghost { opacity: .4; }

        .media-card-footer { padding: .6rem .75rem; border-top: 1px solid var(--media-border); }
        .media-desc-input { width: 100%; background: transparent; border: none; border-bottom: 1px dashed var(--media-border); color: var(--media-text); font-size: .78rem; padding: .2rem 0; outline: none; transition: border-color .2s; }
        .media-desc-input::placeholder { color: var(--media-muted); }
        .media-desc-input:focus { border-color: var(--media-accent); }
        .media-desc-status { font-size: .65rem; color: var(--media-muted); margin-top: .2rem; min-height: .9rem; transition: color .3s; }
        .media-desc-status.saved { color: var(--media-accent2); }
        .media-desc-status.error { color: var(--media-danger); }

        .upload-progress-wrap { display: none; margin-top: .75rem; }
        .upload-progress-wrap.visible { display: block; }
        .upload-bar-track { background: rgba(255,255,255,.08); border-radius: 20px; height: 6px; overflow: hidden; }
        .upload-bar-fill { height: 100%; border-radius: 20px; background: linear-gradient(90deg, var(--media-accent), var(--media-accent2)); width: 0%; transition: width .3s; }
        .upload-bar-label { font-size: .72rem; color: var(--media-muted); margin-top: .3rem; }

        #media-toast { position: fixed; bottom: 2rem; right: 2rem; z-index: 9999; display: flex; flex-direction: column; gap: .5rem; pointer-events: none; }
        .toast-item { background: var(--media-surface); border: 1px solid var(--media-border); border-left: 4px solid var(--media-accent2); color: var(--media-text); padding: .7rem 1.1rem; border-radius: var(--media-radius-sm); font-size: .82rem; box-shadow: var(--media-shadow); animation: toastIn .3s ease both; pointer-events: auto; }
        .toast-item.error { border-left-color: var(--media-danger); }
        @keyframes toastIn { from { opacity:0; transform: translateX(20px); } to { opacity:1; transform: none; } }

        #empty-state { text-align: center; padding: 2rem 1rem; color: var(--media-muted); font-size: .85rem; display: none; }
...
<?php endif; ?>
                ...
                <div class="card shadow-sm mb-3">
                    ...
                    <div class="card-body row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="duration_minutes" class="form-control" value="<?= $tour['duration_minutes'] ?>" min="15">
                        </div>

                        <div class="col-md-4">
                            <label class="form-label">Mínimo de personas</label>
                            <input type="number" name="min_pax" class="form-control" value="<?= $tour['min_pax'] ?>" min="1">
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="difficulty_level" class="form-select">
                                <?php foreach (['easy' => 'Fácil', 'moderate' => 'Moderado', 'hard' => 'Difícil'] as $val => $label): ?>
                                    <option value="<?= $val ?>" <?= $tour['difficulty_level'] === $val ? 'selected' : '' ?>><?= $label ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" name="meeting_point" class="form-control" value="<?= esc($tour['meeting_point'] ?? '') ?>">
                        </div>

                        <div class="col-md-6">
                            <label class="form-label">Política de cancelación</label>
                            <select name="cancellation_policy" class="form-select">
                                <?php foreach (['flexible'=>'Flexible','moderate'=>'Moderada','strict'=>'Estricta','non_refundable'=>'No reembolsable'] as $val => $label): ?>
                                    <option value="<?= $val ?>" <?= $tour['cancellation_policy'] === $val ? 'selected' : '' ?>><?= $label ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="card-body row g-3">
                        <div class="col-12">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" name="price_adult" class="form-control" value="<?= $tour['price_adult'] ?>" required>
                            </div>
                        </div>
                        <div class="col-12">
                        ...
                        <div id="included-list">
                            <?php foreach ($included as $item): ?>
                                <div class="input-group mb-2">
                                    <input type="text" name="included[]" class="form-control form-control-sm" value="<?= esc($item) ?>">
                    ...
                    <div class="card-body">
                        <div id="excluded-list">
                            <?php foreach ($excluded as $item): ?>
                                <div class="input-group mb-2">
                                    <input type="text" name="excluded[]" class="form-control form-control-sm" value="<?= esc($item) ?>">
                                    <button type="button" class="btn btn-sm btn-outline-secondary" onclick="this.parentElement.remove()">×</button>
                                </div>
                            <?php endforeach; ?>
                            <div class="input-group mb-2">
            ...
            <div id="media-grid">
                <?php foreach ($mediaItems as $item):
                    ...
                    <div class="media-card" data-media-id="<?= esc($item['id']) ?>">
                        ...
                        <?php else: ?>
                            <img src="<?= $publicUrl ?>" alt="<?= esc($item['description'] ?? '') ?>" class="media-thumb" loading="lazy">
                        <?php endif; ?>
    ...
    <script>
        const TOUR_ID      = <?= (int)$tour['id'] ?>;
        const CSRF_NAME    = '<?= csrf_token() ?>';

        /**
         * CI4 con $tokenRandomize = true invalida el token en cada POST.
         * Solución: leer el token actualizado desde la cookie que CI4 renueva
         * automáticamente en cada respuesta HTTP. Así cada request siempre
         * usa el token vigente sin necesidad de recargar la página.
         */
        function getToken() {
            const name  = CSRF_NAME;
        ...
        /**
         * Helper central para todos los POST AJAX.
         * Siempre usa FormData (nunca JSON) para que CI4 encuentre el token en $_POST.
         * Acepta un objeto de campos adicionales.
         */
        async function apiPost(url, fields = {}) {
            const fd = new FormData();
            fd.append(CSRF_NAME, getToken());   // token fresco en cada llamada
        ...
        // ── Crear card desde la respuesta JSON del server ──────────────
        ...
        ${isVideo
                ? `<div class="video-thumb"><i class="bi bi-play-circle-fill"></i></div>`
                : `<img src="${url}" alt="" class="media-thumb" loading="lazy">`}
        <span class="media-type-badge ${isVideo ? 'video' : 'image'}">${isVideo ? '▶ Video' : '🖼 Foto'}</span>
        <button type="button" class="btn-delete-media" title="Eliminar"
        ...
        <div class="media-card-footer">
            <input type="text" class="media-desc-input"
                   placeholder="Añadir descripción…" value=""
        ...
        // ── Upload (secuencial: espera respuesta antes del siguiente) ──
        // Así la cookie CSRF ya viene renovada para el siguiente archivo.
        async function uploadFiles(files) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                label.textContent = `Subiendo ${i + 1} de ${files.length}: ${file.name}`;

                // Construir el FormData manualmente para incluir el archivo
                const fd = new FormData();
                fd.append(CSRF_NAME, getToken());   // token fresco desde cookie
                fd.append('file', file);
                ...
                try {
                    const res  = await fetch(`/tours/${TOUR_ID}/media/upload`, {
                        ...
                        headers: { 'X-Requested-With': 'XMLHttpRequest' },
                        body:    fd,
                        // credentials: 'same-origin' ya es el default
                    });
                    const data = await res.json();

                    if (data.success) {
                        document.getElementById('media-grid').appendChild(createMediaCard(data.item));
                        updateBadge();
                        toast(`✓ ${file.name}`);
                        // La cookie ya fue renovada por CI4 en esta respuesta,
                        // getToken() devolverá el nuevo valor en la próxima iteración.
                    } else {
                        toast(`✗ ${file.name}: ${data.error}`, 'error');
                    }
                } catch {
                    toast(`✗ Error de red: ${file.name}`, 'error');
                }
            }

            fill.style.width  = '100%';
            label.textContent = 'Completado';
            setTimeout(() => { wrap.classList.remove('visible'); fill.style.width = '0%'; }, 1400);
        }
        ...
        async function deleteMedia(mediaId, btn) {
            ...
            try {
                const res  = await apiPost(`/tours/${TOUR_ID}/media/${mediaId}/delete`);
                const data = await res.json();
            ...
            descTimers[mediaId] = setTimeout(async () => {
                try {
                    const res  = await apiPost(
                        `/tours/${TOUR_ID}/media/${mediaId}/description`,
        ...
        // ── Reordenar ──────────────────────────────────────────────────
        Sortable.create(document.getElementById('media-grid'), {
            animation:  180,
            ghostClass: 'sortable-ghost',
            handle:     '.drag-handle',
            onEnd: async () => {
                const order = [...document.querySelectorAll('#media-grid .media-card')]
                    .map(c => c.dataset.mediaId);
                try {
                    // order como string JSON dentro del FormData
                    await apiPost(`/tours/${TOUR_ID}/media/reorder`, { order: JSON.stringify(order) });
                } catch {
