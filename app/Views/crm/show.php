...
$segmentDefs = [
    'champion'  => ['label' => 'Champions',      'color' => '#7c3aed', 'bg' => '#f5f3ff'],
    'loyal'     => ['label' => 'Leales',          'color' => '#2563eb', 'bg' => '#eff6ff'],
    'at_risk'   => ['label' => 'En riesgo',       'color' => '#dc2626', 'bg' => '#fff5f5'],
    'potential' => ['label' => 'Alto potencial',  'color' => '#059669', 'bg' => '#f0fdf4'],
    'new'       => ['label' => 'Nuevos',          'color' => '#0891b2', 'bg' => '#f0f9ff'],
    'lost'      => ['label' => 'Perdidos',        'color' => '#94a3b8', 'bg' => '#f8fafc'],
    'regular'   => ['label' => 'Regulares',       'color' => '#64748b', 'bg' => '#f8fafc'],
];
?>
    ...
    <div class="crm-header">
        <div>
            ...
            <p class="text-muted small mb-0">
                Scoring RFM · <?= $stats['total'] ?> huéspedes registrados
            </p>
        </div>
    ...
    <div class="crm-stats">
        <a href="/crm<?= $search ? '?q=' . urlencode($search) : '' ?>"
           ...
           style="--seg-color:#6366f1;--seg-bg:#f0f4ff">
            <div class="cstat-n" style="color:#6366f1"><?= $stats['total'] ?></div>
        ...
        <?php foreach ([
                           ...
                           'potential' => ['Alto potencial', 0,                  '#059669', '#f0fdf4'],
                           'new'       => ['Nuevos',         $stats['new'],      '#0891b2', '#f0f9ff'],
                           'lost'      => ['Perdidos',       $stats['lost'],     '#94a3b8', '#f8fafc'],
                       ] as $seg => [$label, $count, $color, $bg]):
            $url = '/crm?segment=' . $seg . ($search ? '&q=' . urlencode($search) : '');
            ?>
            <a href="<?= $segment === $seg ? '/crm' : $url ?>"
               class="cstat <?= $segment === $seg ? 'active-filter' : '' ?>"
               style="--seg-color:<?= $color ?>;--seg-bg:<?= $bg ?>">
                <div class="cstat-n" style="color:<?= $color ?>">
                    <?= $seg === 'potential'
                        ? count(array_filter($guests ?? [], fn($g) => $g['segment'] === 'potential'))
                        : $count ?>
                </div>
    ...
    <form method="GET" action="/crm" class="crm-toolbar">
        ...
        <select name="sort" class="crm-sort" onchange="this.form.submit()">
            <option value="score"      <?= $sort === 'score'      ? 'selected' : '' ?>>Ordenar: Score</option>
            <option value="last_visit" <?= $sort === 'last_visit' ? 'selected' : '' ?>>Última visita</option>
            <option value="spent"      <?= $sort === 'spent'      ? 'selected' : '' ?>>Mayor gasto</option>
            <option value="visits"     <?= $sort === 'visits'     ? 'selected' : '' ?>>Más visitas</option>
    ...
    <div class="crm-table-wrap">
        <div class="crm-empty">
            ...
            <p class="fw-semibold mb-1">
                <?= $search || $segment ? 'Sin resultados para este filtro' : 'Aún no hay huéspedes registrados' ?>
            </p>
            <p style="font-size:.82rem">
                <?= $search || $segment
                    ? '<a href="/crm">Ver todos los huéspedes</a>'
                    : 'Los huéspedes aparecerán aquí cuando completes tu primera reservación.' ?>
            </p>
        </div>
    </div>
<?php else: ?>
        ...
        <table class="crm-table">
            ...
            <?php foreach ($guests as $g):
                ...
                $initials = implode('', array_map(
                    fn($w) => strtoupper($w[0]),
                    array_slice(explode(' ', $g['full_name']), 0, 2)
                ));
                // Color del avatar basado en segmento
                $avatarColors = [
                    'champion'  => '#7c3aed',
                    'loyal'     => '#2563eb',
                    'at_risk'   => '#dc2626',
                ...
                $avatarColor = $avatarColors[$rfm['segment']] ?? '#6366f1';
                $segDef = $segmentDefs[$rfm['segment']] ?? $segmentDefs['regular'];
                ?>
                <tr>
                    <!-- Huésped -->
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div>
                                <div class="fw-semibold" style="color:#0f172a;font-size:.875rem">
                                    <?= esc($g['full_name']) ?>
                                </div>
                        ...
                        <div class="score-bar">
                            <div class="score-track">
                                <div class="score-fill"
                                     style="width:<?= ($rfm['score'] / 5) * 100 ?>%">
                                </div>
                            </div>
                            <span class="score-num"><?= $rfm['score'] ?></span>
                        </div>
                        <!-- Pills R F M -->
                        <div class="rfm-pills mt-1">
                            <?php
                            $rfmColors = [1=>'#e2e8f0',2=>'#bfdbfe',3=>'#93c5fd',4=>'#3b82f6',5=>'#1d4ed8'];
                            foreach (['r'=>'R','f'=>'F','m'=>'M'] as $key => $lbl):
                                $val = $rfm[$key . '_score'];
                                ?>
                                <div class="rfm-pill"
                                     style="background:<?= $rfmColors[$val] ?>;
                                             color:<?= $val >= 3 ? '#fff' : '#64748b' ?>"
                                     title="<?= $lbl ?>: <?= $val ?>/5">
                                    <?= $lbl ?>
                                </div>
                            <?php endforeach; ?>
                    ...
                    <!-- Visitas -->
                    <td>
                        <span class="fw-semibold"><?= $g['total_reservations'] ?? 0 ?></span>
                    ...
                    <!-- Última visita -->
                    <td>
                        <?php if ($rfm['last_visit']): ?>
                            <div style="font-size:.82rem">
                                <?= date('d M Y', strtotime($rfm['last_visit'])) ?>
                            </div>
                            <div style="font-size:.72rem;color:<?= $rfm['days_since'] > 180 ? '#dc2626' : '#94a3b8' ?>">
                                hace <?= $rfm['days_since'] ?> días
                            </div>
                        <?php else: ?>
                            <span style="color:#94a3b8;font-size:.78rem">Sin visitas</span>
                        <?php endif; ?>
                    </td>
...
<?php endif; ?>
<script>
    document.getElementById('msgPreview')
        ?.addEventListener('input', function () {
            if (guestPhone) {
                const waMsg = encodeURIComponent(this.textContent.trim());
                const waLink = document.getElementById('waLink');
                if (waLink) waLink.href = `https://wa.me/${guestPhone}?text=${waMsg}`;
            }
        });
</script>
<?= $this->endSection() ?>